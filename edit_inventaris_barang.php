...
if(isset($_POST['submit'])) {
  define('MyConst', TRUE);
  include 'function_inventaris_barang.php';
  $inventaris_barang = new inventaris_barang();
  $inventaris_barang -> edit_inventaris_barang($_POST);
}

$id =$_GET['id'];
$query ="SELECT * FROM tblinventaris_brg WHERE id=$id";
$sql = mysqli_query($koneksi,$query);
if (mysqli_num_rows($sql) == 0) {
    header("location:./inventaris_barang.php");
  } else {
                ...
                <div class="md:grid md:grid-cols-3 md:gap-6">
                  <div class="md:col-span-1">
                    <div class="px-4 sm:px-0">
                      <h3 class="text-lg font-medium leading-6 text-gray-900 px-10 px-5">Edit Inventaris Barang</h3>
                    </div>
                  </div>
                    ...
                    <form method="POST" enctype="multipart/form-data">
                      <input type="" class="form-control" id="id" name="id" value=<?php echo $id ?> hidden>
                            <div class="text-gray-700">
                                <label class="block mb-1" for="forms-labelOverInputCode">No Barang</label>
                                <input name="nomor_barang" value="<?= $data['brg'] ?>" class="w-full h-10 px-3 text-base placeholder-gray-600 border rounded-lg focus:shadow-outline" type="text" placeholder="" id="forms-labelOverInputCode"/>
                              </div>
              
                              <div class="text-gray-700">
                                <label class="block mb-1" for="forms-labelOverInputCode">Nama Barang</label>
                                <input name="nama_barang" value="<?= $data['namabrg'] ?>" class="w-full h-10 px-3 text-base placeholder-gray-600 border rounded-lg focus:shadow-outline" type="text" placeholder="Regular input" id="forms-labelOverInputCode"/>
                              </div>
              
                              <div class="text-gray-700">
                                <label class="block mb-1" for="forms-labelOverInputCode">Jenis Barang</label>
                                <input name="jenis_barang" value="<?= $data['jenis'] ?>" class="w-full h-10 px-3 text-base placeholder-gray-600 border rounded-lg focus:shadow-outline" type="text" placeholder="" id="forms-labelOverInputCode"/>
                              </div>
              
                            <div class="col-span-6 sm:col-span-3">
                              <label for="country" class="block text-sm font-medium text-gray-700">Divisi</label>
                              <select name="kondisi" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                <option <?= ($data['kondisi'] == 'Baik') ? 'selected' : '' ?>>Baik</option>
                                <option <?= ($data['kondisi'] == 'Rusak') ? 'selected' : '' ?>>Rusak</option>
                                <option <?= ($data['kondisi'] == 'Retensi') ? 'selected' : '' ?>>Retensi</option>
                              </select>
                            </div>
              
                            <div class="text-gray-700">
                                <label class="block mb-1" for="forms-labelOverInputCode">Jumlah Barang</label>
                                <input name="jumlah_barang" value="<?= $data['jumlah'] ?>" class="w-full h-10 px-3 text-base placeholder-gray-600 border rounded-lg focus:shadow-outline" type="text" placeholder="" id="forms-labelOverInputCode"/>
                              </div>
                        ...
                        <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
                            <button type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-indigo bg-white-600 hover:bg-indigo-700 ">
                               <a href="inventaris_barang.php">Cancel</a>
                              </button>
